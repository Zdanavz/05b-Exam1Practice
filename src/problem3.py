...
def run_test_problem3a():
    ...
    # Window 3:
    title = 'Problem 3a.  Test 5: Start at (30, 10), 5 lines.'
    window4 = rg.RoseWindow(550, 200, title)

    # Test 5 (it is on window 4):
    point = rg.Point(30, 10)
    expected = 25
...
def problem3a(window, point, n):
    ...
    sum=0
    for k in range(n):
        l1=rg.Line(rg.Point(point.x+20*k,point.y+k*10),rg.Point(point.x+20*k,point.y+k*10+50))
        l1.thickness=1+k*2
        if l1.thickness>=13:
            l1.thickness=13
        l1.attach_to(window)
        sum=sum+l1.thickness
    window.render()
    return sum
...
def problem3b(m, point1):
    ...
    y=point1.y
    for k in range(m):
        sum=sum+problem3a(window,rg.Point(point1.x,y),3+2*k)
        y = y + 60
